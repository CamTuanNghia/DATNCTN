...
using WebSiteMCSport.Models.Payments;
using System;
            ...
            if (ModelState.IsValid)
            {
                ...
                if (cart != null)
                {
                    ...
                    foreach (var sp in cart.Items)
                    {
                        ...
                        strSanPham += "<td>" + WebSiteMCSport.Common.Common.FormatNumber(sp.TotalPrice, 0) + "</td>";
                        strSanPham += "</tr>";
                    ...
                    contentCustomer = contentCustomer.Replace("{{ThanhTien}}", WebSiteMCSport.Common.Common.FormatNumber(thanhtien, 0));
                    contentCustomer = contentCustomer.Replace("{{TongTien}}", WebSiteMCSport.Common.Common.FormatNumber(TongTien, 0));
                    WebSiteMCSport.Common.Common.SendMail("ShopOnline", "Đơn hàng #" + order.Code, contentCustomer.ToString(), req.Email);
                    ...
                    contentAdmin = contentAdmin.Replace("{{ThanhTien}}", WebSiteMCSport.Common.Common.FormatNumber(thanhtien, 0));
                    contentAdmin = contentAdmin.Replace("{{TongTien}}", WebSiteMCSport.Common.Common.FormatNumber(TongTien, 0));
                    WebSiteMCSport.Common.Common.SendMail("ShopOnline", "Đơn hàng mới #" + order.Code, contentAdmin.ToString(), ConfigurationManager.AppSettings["EmailAdmin"]);
                    cart.ClearCart();
